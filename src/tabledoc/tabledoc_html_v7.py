...
def get_modules_and_standard_status(python_executable):
    script = """
import pkgutil
import sys
import importlib.util

modules = [modname for importer, modname, ispkg in pkgutil.iter_modules()]
std_lib_modules = [mod for mod in modules if (mod in sys.builtin_module_names or
    (importlib.util.find_spec(mod) is not None and 'site-packages' not in importlib.util.find_spec(mod).origin))]
print(modules)
print(std_lib_modules)
"""
    try:
        output = subprocess.check_output([python_executable, "-c", script], text=True)
        modules, std_lib_modules = output.split("\n")[:2]
        return eval(modules), eval(std_lib_modules)
    except subprocess.CalledProcessError as e:
        print(f"Error getting modules from installation {python_executable}: {e}")
        return [], []
...
def get_module_documentation_links(python_executable, modules):
    script = f"""
modules = {modules}
doc_links = {{}}
for mod in modules:
    try:
        ...
        if hasattr(module, '__doc__') and module.__doc__:
            doc_links[mod] = module.__doc__.split('\\n')[0]
    except ImportError:
        pass
print(doc_links)
"""
...
def generate_table_for_module(module_name, exclude_underscore, python_executable, doc_links, std_modules):
    try:
            ...
            for j in range(columns):
                if i + j < len(items):
                    ...
                    if module_name in std_modules:
                        ...
                        doc_link = doc_links.get(module_name) or f"#{item}"
...
def main():
    ...
    all_modules, std_modules = get_modules_and_standard_status(selected_python)
    ext_modules = sorted([mod for mod in all_modules if mod not in std_modules])
    std_modules = sorted(std_modules)
    doc_links = get_module_documentation_links(selected_python, all_modules)
    errors = []
    ...
    with open("python_modules_tables.html", "w") as f:
        ...
        for module_name in std_modules + ext_modules:
            if args.exclude_underscore and module_name.startswith("_"):
                continue
            functions_table, classes_table, error = generate_table_for_module(module_name, args.exclude_underscore_functions, selected_python, doc_links, std_modules)
