...
def get_modules_from_installation(python_executable):
    try:
        output = subprocess.check_output([python_executable, "-c", 
                                          "import pkgutil; print([modname for _, modname, _ in pkgutil.iter_modules()])"], 
                                          text=True)
        return eval(output)
...
def is_standard_library(module_name, python_executable):
    script = f"""
import sys
...
if '{module_name}' in sys.builtin_module_names:
    ...
    if spec is not None and spec.origin is not None:
        print('site-packages' not in spec.origin)
    else:
        print(False)
"""
    try:
        output = subprocess.check_output([python_executable, "-c", script], text=True).strip()
        return output == "True"
    except subprocess.CalledProcessError:
        return False

def get_module_documentation_link(module_name, python_executable):
    script = f"""
try:
    module = __import__('{module_name}')
    if hasattr(module, '__doc__') and module.__doc__:
        print(module.__doc__.split('\\n')[0])
except ImportError:
    pass
"""
    try:
...
def generate_table_for_module(module_name, exclude_underscore, python_executable):
    try:
    ...
    def create_table(items, item_type):
        ...
        for i in range(0, len(items), columns):
            ...
            for j in range(columns):
                if i + j < len(items):
                    item = items[i + j]
                    if is_standard_library(module_name, python_executable):
                        doc_link = f"https://docs.python.org/3/library/{module_name}.html#{module_name}.{quote(item)}"
                    else:
                        doc_link = get_module_documentation_link(module_name, python_executable) or f"#{item}"
...
def main():
    ...
    all_modules = get_modules_from_installation(selected_python)
    std_modules = sorted([mod for mod in all_modules if is_standard_library(mod, selected_python)])
    ext_modules = sorted([mod for mod in all_modules if not is_standard_library(mod, selected_python)])
    errors = []
    ...
    with open("python_modules_tables.html", "w") as f:
        ...
        for module_name in std_modules + ext_modules:
            if args.exclude_underscore and module_name.startswith("_"):
                continue
            functions_table, classes_table, error = generate_table_for_module(module_name, args.exclude_underscore_functions, selected_python)
